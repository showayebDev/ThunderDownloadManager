...
type EngineConfig struct {
	...
	ProxyConfig                 proxy.ProxyConfig  `json:"proxyConfig"`
}
...
func applyEngineSettings(cfg EngineConfig) {
	...
	if cfg.GlobalSpeedLimiter && cfg.GlobalSpeedLimit > 0 {
		limiter.GlobalLimiter.SetLimit(cfg.GlobalSpeedLimit)
	} else {
		limiter.GlobalLimiter.SetLimit(0) // Unlimited
	}

	// 2. Update ProxyManager
	if proxy.GlobalProxyManager != nil {
		proxy.GlobalProxyManager.SetConfig(cfg.ProxyConfig)
	}
