...
func DownloadChunk(ctx context.Context, url string, chunk *core.ChunkState, file *os.File, client *http.Client, taskLimiter *limiter.SpeedLimiter, onFallback func(*core.ChunkState), opts ...string) error {
	if client == nil {
	...
	buf := make([]byte, 64*1024) // 64KB read/write buffer
	var lastErr error
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		...
		if endByte > 0 && currentByte > endByte {
			chunk.SetStatus(core.StatusFinished)
			return nil
		}
		...
		if err != nil {
			...
			chunk.SetStatus(core.StatusError)
			return err
		}
		...
		if cookies != "" && (forceCookie || !core.ShouldBypassCookies(url, "http")) {
		...
		for {
			...
			if n > 0 {
				...
				if taskLimiter != nil && taskLimiter.IsExplicit() {
					if taskLimiter.GetLimit() > 0 {
						if waitErr := taskLimiter.Wait(ctx, n); waitErr != nil {
							readWatchdog.Stop()
							resp.Body.Close()
							return waitErr
						}
					...
					if taskLimiter != nil && taskLimiter.GetLimit() > 0 {
						if waitErr := taskLimiter.Wait(ctx, n); waitErr != nil {
							readWatchdog.Stop()
							resp.Body.Close()
							return waitErr
					...
					if gl := limiter.GlobalLimiter; gl != nil && gl.GetLimit() > 0 {
