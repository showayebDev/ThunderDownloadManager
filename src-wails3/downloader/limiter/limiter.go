...
// GlobalLimiter is the shared global token-bucket bandwidth limiter across all download tasks.
var GlobalLimiter = NewSpeedLimiter(0)
...
func (lr *speedLimitedReader) Read(p []byte) (int, error) {
	...
	if n > 0 {
		if lr.limiter != nil && lr.limiter.IsExplicit() {
			...
			if gl := GlobalLimiter; gl != nil && gl.GetLimit() > 0 {
