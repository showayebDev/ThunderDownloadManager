...
func TestSanitizeFilename(t *testing.T) {
	...
	sanitized := core.SanitizeFilename(fbTitle)
	...
	ytSanitized := core.SanitizeFilename(ytTitle)
	...
	if res := core.SanitizeFilename("test:file?.mkv"); res != "test_file_.mkv" {
		t.Errorf("Expected test_file_.mkv, got %s", res)
	}
	if res := core.SanitizeFilename(""); res != "download" {
...
func TestOwnToolsIsolation(t *testing.T) {
	// Verify GetYTDLPExecutable prioritizes ThunderDM's own binary locations when installed
	...
	// Verify GetFFmpegExecutable prioritizes ThunderDM's own binary locations when installed
	...
	// Verify GetFFprobeExecutable prioritizes ThunderDM's own binary locations when installed
