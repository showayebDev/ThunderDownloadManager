...
type TorrentTaskController struct {
	...
	errorMessage string
	options      core.DownloadExtraOptions
...
func NewTorrentTaskController(
	...
	return &TorrentTaskController{
		...
		chunks:     make([]core.ChunkPayload, 0),
	}
}

func (c *TorrentTaskController) Start() {
	c.mu.Lock()
	c.status = core.StatusDownloading
	c.errorMessage = ""
	c.mu.Unlock()
	...
	rawSource := core.CleanTorrentSource(c.options.CleanURL)
	if rawSource == "" {
		rawSource = core.CleanTorrentSource(c.url)
	}
	...
	if t.Info() != nil {
		c.mu.Lock()
		if t.Name() != "" {
			c.filename = core.SanitizeFilename(t.Name())
		}
	...
	for {
		...
		case <-ticker.C:
			...
			if currentStatus != core.StatusDownloading {
				return
			}
			...
			var chunkPayloads []core.ChunkPayload
			...
			if t.Info() != nil {
				...
				if numPieces > 0 {
					...
					chunkPayloads = make([]core.ChunkPayload, numChunks)
					...
					for i := 0; i < numChunks; i++ {
						...
						var chunkStatus core.DownloadStatus = core.StatusPending
						if completedInChunk == totalInChunk && totalInChunk > 0 {
							chunkStatus = core.StatusFinished
						} else if completedInChunk > 0 {
							chunkStatus = core.StatusDownloading
						}

						chunkPayloads[i] = core.ChunkPayload{
							ID:         i + 1,
							Status:     chunkStatus,
				...
				for i := 0; i < numChunks; i++ {
					var st core.DownloadStatus = core.StatusPending
					if i < activePeers {
						st = core.StatusDownloading
					}
					chunkPayloads[i] = core.ChunkPayload{
						ID:         i + 1,
						Status:     st,
...
func (c *TorrentTaskController) completeTask() {
	...
	_ = storage.UpdateDownloadProgress(c.id, c.totalSize, c.totalSize, string(core.StatusFinished), "", nil)
...
func (c *TorrentTaskController) failTask(msg string) {
	c.mu.Lock()
	c.status = core.StatusError
	c.errorMessage = msg
	c.speed = 0
	c.mu.Unlock()

	log.Printf("[TorrentTask] Error on task %s: %s\n", c.id, msg)
	_ = storage.UpdateDownloadProgress(c.id, c.downloaded, c.totalSize, string(core.StatusError), msg, nil)
	c.emitProgress()
}
...
func (c *TorrentTaskController) emitProgress() {
	...
	core.EmitProgressUpdate(id, fn, dl, tot)
...
func (c *TorrentTaskController) Pause() error {
	c.mu.Lock()
	if c.status == core.StatusFinished {
		c.mu.Unlock()
		return nil
	}
	c.status = core.StatusPaused
	c.speed = 0
	c.eta = 0
	if c.cancel != nil {
		c.cancel()
	...
	_ = storage.UpdateDownloadProgress(c.id, c.downloaded, c.totalSize, string(core.StatusPaused), "", nil)
	c.emitProgress()
	return nil
}
...
func (c *TorrentTaskController) Cancel() error {
	...
	_ = storage.UpdateDownloadProgress(c.id, c.downloaded, c.totalSize, string(core.StatusCanceled), "", nil)
	c.emitProgress()
	return nil
}
