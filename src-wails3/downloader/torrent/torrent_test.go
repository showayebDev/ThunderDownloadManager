...
func TestIsTorrentURL(t *testing.T) {
	...
	for _, tt := range tests {
		result := core.IsTorrentURL(tt.url)
...
func TestIsTorrentFile(t *testing.T) {
	...
	for _, tt := range tests {
		result := core.IsTorrentFile(tt.path)
		if result != tt.expected {
...
func TestNormalizeSavePathForTorrent(t *testing.T) {
	p := core.NormalizeSavePath("", "Torrents")
	if p == "" {
...
func TestNewTorrentTaskController_NilContext(t *testing.T) {
	...
	task := NewTorrentTaskController(nil, "torrent-test-1", "magnet:?xt=urn:btih:0123456789abcdef0123456789abcdef01234567", "C:\\Downloads", "test.iso", 8, nil, core.DownloadExtraOptions{})
	if task == nil {
...
func TestNewThunderTorrentStorage(t *testing.T) {
	...
	st := NewThunderTorrentStorage(tempDir)
	if st == nil {
...
func TestTorrentTaskControllerPauseResume(t *testing.T) {
	...
	task := NewTorrentTaskController(
		...
		8,
		nil,
		core.DownloadExtraOptions{Protocol: "Torrent"},
	)
	...
	task.mu.Lock()
	task.status = core.StatusDownloading
	...
	st := task.GetState()
	if st["status"] != core.StatusPaused {
		t.Errorf("expected status %v, got %v", core.StatusPaused, st["status"])
	}
