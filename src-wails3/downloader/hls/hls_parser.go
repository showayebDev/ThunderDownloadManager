// hls_parser.go parses HLS (HTTP Live Streaming) M3U8 master and media playlists,
// resolves relative segment/variant URLs, and selects optimal stream variants.
package hls
...
func FetchAndParseHLS(ctx context.Context, client *http.Client, rawURL string, opts ...string) (*HLSPlaylist, error) {
	...
	if cookies != "" && (forceCookie || !core.ShouldBypassCookies(rawURL, "hls")) {
